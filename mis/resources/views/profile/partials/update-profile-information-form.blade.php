<section id="mis-profile-edit" class="rounded-2xl border border-gray-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/40 p-4 space-y-4">
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-slate-400">
            {{ __('Update your name, contact details, photo, and email. Changing your email may require verification again.') }}
        </p>
    </header>
        ...
        <div class="rounded-lg border border-gray-200 dark:border-slate-700 p-3 space-y-2">
            ...
            <div class="flex flex-wrap items-center gap-3">
                <img
                    ...
                    alt=""
                    class="h-14 w-14 rounded-full border border-gray-200 object-cover dark:border-slate-700"
                    width="56"
                    height="56"
                    loading="lazy"
                    referrerpolicy="no-referrer"
                >
                <input type="file" name="avatar" accept="image/*" class="block max-w-full text-xs text-gray-600 dark:text-slate-400 file:mr-2 file:rounded file:border-0 file:bg-gray-200 file:px-2 file:py-1 dark:file:bg-slate-700">
            </div>
            @if ($user->avatar_path)
                <label class="inline-flex items-center gap-2 text-xs text-gray-600 dark:text-slate-400">
                    <input type="checkbox" name="remove_avatar" value="1" class="rounded border-gray-300 dark:border-slate-600">
                    {{ __('Remove uploaded photo (use Gravatar / default)') }}
                </label>
            @endif
