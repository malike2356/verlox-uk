...
<section class="rounded-2xl border border-gray-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/40 p-4 space-y-3">
    ...
    @elseif ($sessions->isEmpty())
        <p class="text-sm text-gray-600 dark:text-slate-400">{{ __('No session records were found for your account.') }}</p>
    @else
        <p class="text-sm text-gray-600 dark:text-slate-400">
            {{ __('Devices and browsers where you are signed in. Sign out everywhere else if you lose a device or see suspicious activity.') }}
        </p>
        <ul class="divide-y divide-gray-200 dark:divide-slate-800 rounded-lg border border-gray-200 dark:border-slate-800">
            @foreach ($sessions as $row)
                ...
                @endphp
                <li class="flex flex-wrap items-start justify-between gap-2 px-3 py-2.5 text-sm">
                    <div class="min-w-0">
        ...
        @if ($sessions->count() > 1)
            <form method="post" action="{{ route('profile.sessions.logout-others') }}" class="space-y-3 rounded-lg border border-amber-200 bg-amber-50/80 p-3 dark:border-amber-900/50 dark:bg-amber-950/20">
                @csrf
                <p class="text-sm text-gray-800 dark:text-amber-100/90">{{ __('Sign out all sessions except this browser.') }}</p>
                <div>
                    <label class="text-xs text-gray-600 dark:text-slate-400" for="sessions_logout_password">{{ __('Confirm your password') }}</label>
                    <x-password-input id="sessions_logout_password" name="password" class="mt-1 block w-full max-w-md" required autocomplete="current-password" />
                    <x-input-error :messages="$errors->get('password')" class="mt-2" />
                </div>
                <button type="submit" class="inline-flex items-center rounded-lg border border-amber-800 bg-amber-700 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-white hover:bg-amber-800 dark:border-amber-700 dark:bg-amber-900 dark:hover:bg-amber-800">
